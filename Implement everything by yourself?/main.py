...
def unhash(hashed_string):
    ...
    h664 = ""
    for i in range(0, 16):
        h664 += (hashed_string[i*2])
        h665 += (hashed_string[i*2+1])
    ...
    for i in range(16 - 1, 0 - 1, -1):
        possibilities = reverse_f2(ord(h664[i]), sta[i])
        ...
        for key in possible_substitutions:
            for h664 in possible_h664s:
                for possibility in possible_substitutions[key]:
                    possible_h664s_tmp.add(
                        h664[:key] + chr(possibility) + h664[key+1:])
    ...
    possible_input_strings = set(p + h665 for p in possible_h664s)
...
def unhash_underscore(hashed_string):
    ...
    unhashed_strings_list.sort(
        key=lambda x: re.findall(r"[^a-zA-Z0-9\s]", x).__len__())
...
def main():
    intput_string = input(
        "Enter the hashed string but without the '{ ' and ' }' that might be at the beginning and end of the given string:\n")
